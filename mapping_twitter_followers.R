#######https://github.com/LucasPuente/geocoding/blob/master/mapping_twitter_followers.R
...
ratcliffe <- getUser("RepRatcliffe")
ratcliffe_tx <- getUser("RatcliffeTX4")
...
ratcliffe_follower_IDs<-ratcliffe$getFollowers(retryOnRateLimit=250)
length(ratcliffe_follower_IDs)
#to get who the account is following....
ratcliffe_friend_ids <-ratcliffe$getFriends(retryOnRateLimit=250)
length(ratcliffe_friend_ids)
TXratcliffe_friend_ids <-ratcliffe_tx$getFriends(retryOnRateLimit=250)
length(TXratcliffe_friend_ids)
#######
...
ratcliffe_followers_df = rbindlist(lapply(ratcliffe_follower_IDs,as.data.frame))

#turn friend list into df
ratcliffe_friends_df = rbindlist(lapply(ratcliffe_friend_ids,as.data.frame))
TXratcliffe_friend_DF = rbindlist(lapply(TXratcliffe_friend_ids,as.data.frame))
